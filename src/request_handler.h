 ...
 * API request handler
 */
boolean requestJsonApi(JsonDocument &doc, String url, String payload = "",
                       size_t capacity = 0, String type = "POST",
...
void handleRoot() {
  ...
  s += "<link href=\"https://fonts.googleapis.com/css?family=Press+Start+2P\" "
       "rel=\"stylesheet\">";
  s += "<link href=\"https://unpkg.com/nes.css@2.3.0/css/nes.min.css\" "
       "rel=\"stylesheet\" />";
  ...
  s += "      document.getElementById('btn_open').href = "
       "data.verification_uri;\n";
  s += "      document.getElementById('lbl_message').innerText = "
       "data.message;\n";
  ...
  if (strlen(paramTenantValue) == 0 || strlen(paramClientIdValue) == 0) {
    s += "<p class=\"note nes-text is-error\">Some settings are missing. Go to "
         "<a href=\"config\">configuration page</a> to complete "
         "setup.</p></div>";
  } else {
    if (access_token == "") {
      s += "<p class=\"note nes-text is-error\">No authentication infos found, "
           "start device login flow to complete widget setup!</p></div>";
    } else {
      s +=
          "<p class=\"note nes-text\">Device setup complete, but you can start "
          "the device login flow if you need to re-authenticate.</p></div>";
    }
    s += "<div><button type=\"button\" class=\"nes-btn\" "
         "onclick=\"openDeviceLoginModal()\">Start device login</button></div>";
  }
  ...
  s += "<button id=\"btn_close\" class=\"nes-btn\" "
       "onclick=\"closeDeviceLoginModal()\">Close</button>\n";
  s += "<a class=\"nes-btn is-primary ml-s\" id=\"btn_open\" "
       "href=\"https://microsoft.com/devicelogin\" target=\"_blank\">Open "
       "device login</a>\n";
  s += "</menu>\n";
  s += "</dialog>\n";
  s += "</section>\n";
  ...
  s += "</div>";
  s += "<section class=\"nes-container with-title\"><h3 "
       "class=\"title\">Current settings</h3>";
  s += "<div class=\"nes-field mt-s\"><label "
       "for=\"name_field\">Client-ID</label><input type=\"text\" "
       "id=\"name_field\" class=\"nes-input\" disabled value=\"" +
       String(paramClientIdValue) + "\"></div>";
  s += "<div class=\"nes-field mt-s\"><label for=\"name_field\">Tenant "
       "hostname / ID</label><input type=\"text\" id=\"name_field\" "
       "class=\"nes-input\" disabled value=\"" +
       String(paramTenantValue) + "\"></div>";
  s += "<div class=\"nes-field mt-s\"><label for=\"name_field\">Polling "
       "interval (sec)</label><input type=\"text\" id=\"name_field\" "
       "class=\"nes-input\" disabled value=\"" +
       String(paramPollIntervalValue) + "\"></div>";
  s += "<div class=\"nes-field mt-s\"><label for=\"name_field\">Number of "
       "LEDs</label><input type=\"text\" id=\"name_field\" class=\"nes-input\" "
       "disabled value=\"" +
       String(paramNumLedsValue) + "\"></div>";
  s += "<div class=\"nes-field mt-s\"><label for=\"name_field\">LED Data Pin "
       "(GPIO)</label><input type=\"text\" id=\"name_field\" "
       "class=\"nes-input\" "
       "disabled value=\"" +
       String(paramDataPinValue) + "\"></div>";
  s += "</section>";

  s += "<section class=\"nes-container with-title mt\"><h3 "
       "=\"title\">Memory usage</h3>";
  s +=
      "<div>Sketch: " + String(ESP.getFreeSketchSpace() - ESP.getSketchSize()) +
      " of " + String(ESP.getFreeSketchSpace()) + " bytes free</div>";
  s += "<progress class=\"nes-progress\" value=\"" +
       String(ESP.getSketchSize()) + "\" max=\"" +
       String(ESP.getFreeSketchSpace()) + "\"></progress>";
  s += "<div class=\"mt-s\">RAM: " + String(ESP.getFreeHeap()) +
       " of 327680 bytes free</div>";
  s += "<progress class=\"nes-progress\" value=\"" +
       String(327680 - ESP.getFreeHeap()) + "\" max=\"327680\"></progress>";
  s += "</section>";

  s += "<section class=\"nes-container with-title mt\"><h3 "
       "class=\"title\">Danger area</h3>";
  s += "<dialog class=\"nes-dialog is-rounded\" id=\"dialog-clearsettings\">\n";
  s += "<p class=\"title\">Really clear all settings?</p>\n";
  s += "<button class=\"nes-btn\" "
       "onclick=\"document.getElementById('dialog-clearsettings').close()\">"
       "Close</button>\n";
  s += "<button class=\"nes-btn is-error\" "
       "onclick=\"performClearSettings()\">Clear all settings</button>\n";
  s += "</dialog>\n";
  s += "<dialog class=\"nes-dialog is-rounded\" "
       "id=\"dialog-clearsettings-result\">\n";
  s += "<p class=\"title\">All settings were cleared.</p>\n";
  s += "</dialog>\n";
  s += "<div><button type=\"button\" class=\"nes-btn is-error\" "
       "onclick=\"document.getElementById('dialog-clearsettings').showModal();"
       "\">Clear all settings</button></div>";
  s += "</section>";

  s += "<div class=\"mt\"><i class=\"nes-icon github\"></i> Find the <a "
       "href=\"https://github.com/toblum/ESPTeamsPresence\" "
       "target=\"_blank\">ESPTeamsPresence</a> project on GitHub.</i></div>";
...
void handleClearSettings() {
  ...
  server.send(200, "application/json",
              F("{\"action\": \"clear_settings\", \"error\": false}"));
  ESP.restart();
}
...
boolean formValidator() {
  ...
  if (l1 < 36) {
    paramClientId.errorMessage =
        "Please provide at least 36 characters for the client id!";
    valid = false;
  }

  int l2 = server.arg(paramTenant.getId()).length();
  if (l2 < 10) {
    paramTenant.errorMessage =
        "Please provide at least 10 characters for the tenant host / GUID!";
    valid = false;
  }

  int l3 = server.arg(paramPollInterval.getId()).length();
  if (l3 < 1) {
    paramPollInterval.errorMessage =
        "Please provide a value for the presence poll interval!";
    valid = false;
  }

  int l4 = server.arg(paramNumLeds.getId()).length();
  if (l4 < 1) {
    paramNumLeds.errorMessage =
        "Please provide a value for the number of LEDs!";
    valid = false;
  }
...
void onConfigSaved() {
  ...
  int newNumLeds = atoi(paramNumLedsValue);

  // Validate data pin
  if (newDataPin < 0 || newDataPin > 16) {
    DBG_PRINTLN(F("Invalid data pin, keeping current."));
    newDataPin = dataPin;
  }

  // Validate LED count
  if (newNumLeds < 1 || newNumLeds > 500) {
    DBG_PRINTLN(F("Invalid LED count, keeping current."));
    newNumLeds = numberLeds;
  }

  // Check if strip needs to be recreated (pin changed or doesn't exist)
  if (!strip || newDataPin != dataPin || newNumLeds != numberLeds) {
    Serial.printf("Reinitializing NeoPixel strip: %d LEDs on GPIO %d\n",
                  newNumLeds, newDataPin);
    ...
    boolean res =
        requestJsonApi(doc,
                       "https://login.microsoftonline.com/" +
                           String(paramTenantValue) + "/oauth2/v2.0/devicecode",
                       "client_id=" + String(paramClientIdValue) +
                           "&scope=offline_access%20openid%20Presence.Read",
                       capacity);

    if (res && doc.containsKey("device_code") && doc.containsKey("user_code") &&
        doc.containsKey("interval") && doc.containsKey("verification_uri") &&
      ...
      responseDoc["verification_uri"] =
          doc["verification_uri"].as<const char *>();
      ...
      server.send(500, "application/json",
                  F("{\"error\": \"devicelogin_unknown_response\"}"));
    }
  } else {
    server.send(409, "application/json",
                F("{\"error\": \"devicelogin_already_running\"}"));
  }
}
