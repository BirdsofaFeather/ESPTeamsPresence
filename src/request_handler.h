...
void handleRoot() {
  ...
  s += "\u003cdiv class=\"nes-field mt-s\"\u003e\u003clabel "
       "for=\"name_field\"\u003eNumber of "
       "LEDs\u003c/label\u003e\u003cinput type=\"text\" id=\"name_field\" "
       "class=\"nes-input\" "
       "disabled value=\"" +
       String(paramNumLedsValue) + "\"\u003e\u003c/div\u003e";
  s += "\u003cdiv class=\"nes-field mt-s\"\u003e\u003clabel "
       "for=\"name_field\"\u003eLED Data Pin "
  ...
  s += "<div class=\"mt-s\">RAM: " + String(ESP.getFreeHeap()) +
       " of 400000 bytes free</div>";
  s += "<progress class=\"nes-progress\" value=\"" +
       String(400000 - ESP.getFreeHeap()) + "\" max=\"400000\"></progress>";
#else
  s += "<div class=\"mt-s\">RAM: " + String(ESP.getFreeHeap()) +
...
void onConfigSaved() {
  ...
  int newLedCount = atoi(paramNumLedsValue);
  if (newLedCount < 1) {
    newLedCount = NUMLEDS;
  }
  ...
  // Note: Data pin can't be changed at runtime, requires ESP restart
  if (newLedCount != numberLeds && strip != nullptr) {
    delete strip;
    numberLeds = newLedCount;
    strip = new NeoPixelBusType(numberLeds, newDataPin);
    strip->Begin();
    Serial.printf("NeoPixelBus reinitialized: %d LEDs on GPIO %d\n", numberLeds,
                  newDataPin);
  }
}
