...
#define RMT_TICK (RMT_CLK_DIV * 1000 / APB_CLK_MHZ) // 50ns for ESP32-C3
#else
...
static void IRAM_ATTR u8_to_rmt(const void *src, rmt_item32_t *dest,
  ...
  const rmt_item32_t bit0 = {
      {{T1_TICKS, 1, T2_TICKS + T3_TICKS, 0}}}; // Logical 0
  const rmt_item32_t bit1 = {
      {{T1_TICKS + T2_TICKS, 1, T3_TICKS, 0}}}; // Logical 1
  const rmt_item32_t reset = {
      {{RESET_TICKS / 2, 0, RESET_TICKS / 2, 0}}}; // Reset
  size_t size = 0;
  size_t num = 0;
  ...
  while (size < src_size && num < wanted_num) {
    if (size <
        src_size - 1) { // have more pixel data, so translate into RMT items
      (pdest++)->val = (*psrc & 0x80) ? bit1.val : bit0.val;
      (pdest++)->val = (*psrc & 0x40) ? bit1.val : bit0.val;
      (pdest++)->val = (*psrc & 0x20) ? bit1.val : bit0.val;
      (pdest++)->val = (*psrc & 0x10) ? bit1.val : bit0.val;
      (pdest++)->val = (*psrc & 0x08) ? bit1.val : bit0.val;
      (pdest++)->val = (*psrc & 0x04) ? bit1.val : bit0.val;
      (pdest++)->val = (*psrc & 0x02) ? bit1.val : bit0.val;
      (pdest++)->val = (*psrc & 0x01) ? bit1.val : bit0.val;
      num += 8;
...
#ifdef ESP32C3
  // ESP32-C3 requires explicit clock source configuration
  config.flags = 0;
#endif
